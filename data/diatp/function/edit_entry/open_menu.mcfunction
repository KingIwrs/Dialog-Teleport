...
execute unless data storage diatp:temp edit_entry.actions[0] run data modify storage diatp:temp edit_entry.actions set value [{"label": {"text":"+","color":"yellow"},"width": 150, "action": {"type": "run_command","command": "trigger diatp.new_entry"}}]
